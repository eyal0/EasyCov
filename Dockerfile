...
RUN cd /root && wget https://github.com/git/git/archive/v2.24.1.tar.gz && tar xzvf v2.24.1.tar.gz && cd git-2.24.1 && make prefix=/usr/local all && make prefix=/usr/local install && cd .. && rm -rf git-2.24.1 && hash -r
